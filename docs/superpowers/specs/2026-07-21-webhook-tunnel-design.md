...
## Addendum (2026-09-16): payment/refund/capture/chargeback events moved to public beta

Mollie moved `payment.*`, `refund.*`, `capture.*`, and `chargeback.*` Next-gen webhook event types out of restricted (support-request-gated) beta into public beta, all under the `payments.read` permission. `GlobalEventPermissions` (`internal/webhookevents/events.go`) now includes all 16 of these event types, so `webhook-tunnel` subscribes to them by default whenever the credential has `payments.read`. `dispute.*`, `file.*`, `unmatched-credit-transfer.*`, and `connect-balance-transfer.*` remain support-request-gated and stay excluded.
